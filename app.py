...
def session_status():
    """
    Endpoint untuk memeriksa status login dari sisi server.
    Ini adalah satu-satunya "sumber kebenaran" untuk status login.
    """
    if 'user_id' in session:
        # Jika ada user_id di sesi server, berarti pengguna valid
        return jsonify({
            'logged_in': True,
            'username': session.get('username', 'Pengguna') # Mengambil username dari sesi
        })
    else:
        # Jika tidak ada, pengguna tidak login
    ...
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session:
            if request.path.startswith('/api/'):
                return jsonify({"success": False, "message": "Kamu harus login untuk mengakses ini."}), 401
...
def admin_galeri_tambah():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return jsonify({
                ...
                'redirect_url': url_for('admin_galeri')
            })
        except Exception as e:
            app.logger.error(f"Gagal menyimpan dokumen ke DB: {e}")

            # Membersihkan file yang sudah ter-upload jika insert DB gagal
    ...
    # Kode untuk request GET tidak berubah
    return render_template('admin/galeri_tambah.html',
                           layanan=layanan,
                           lokasi=lokasi,
                           current_route=request.path,
                           _sa_status=request.args.get('_sa_status'),
...
def admin_pesanan_tambah():
    if request.method == 'POST':
        try:
            ...
            nama_alternatif = request.form['nama_alternatif']
            telepon_alternatif = request.form['telepon_alternatif']
            ...
            pesanan_doc = {
                ...
                'status_pesanan': 'Belum Pemotretan',
...
def admin_pesanan_update(pesanan_id):
    ...
    # Hitung ulang sisa_bayar sesuai dengan status pembayaran
    if status_pembayaran_deposit == 'Lunas':
        if status_pembayaran_pelunasan == 'Lunas' and status_sisa_pembayaran == 'Lunas':
            recalculated_sisa_bayar = recalculated_total_harga - (last_pelunasan_attempt_amount + last_sisa_bayar_attempt_amount)
        elif status_pembayaran_pelunasan == 'Belum Bayar' and status_sisa_pembayaran == 'Lunas':
            recalculated_sisa_bayar = recalculated_total_harga - (deposit_paket + last_sisa_bayar_attempt_amount)
        elif status_pembayaran_pelunasan == 'Lunas' and status_sisa_pembayaran == 'Belum Bayar':
            recalculated_sisa_bayar = recalculated_total_harga - last_pelunasan_attempt_amount
        elif status_pembayaran_pelunasan == 'Belum Bayar' and status_sisa_pembayaran == 'Belum Bayar':
            recalculated_sisa_bayar = recalculated_total_harga - deposit_paket
        else:
            recalculated_sisa_bayar = 0
    else:
        recalculated_sisa_bayar = recalculated_total_harga
    ...
    if status in ['Selesai', 'Sudah Kirim File & Album']:
...
def admin_kirim_pengingat():
    ...
    if status == 'Belum Pemotretan':
        ...
        pesan = f"Halo {nama}, status pesanan kamu saat ini: {status}. Tidak ada pengingat spesifik."
...
def ripe_menunggu_konfirmasi():
    user_id = session.get('user_id')
    if not user_id:
        flash("Kamu harus login untuk melihat riwayat pemesanan.", "warning")
        return redirect(url_for('masuk'))
    ...
    try:
        ...
        flash(f"Terjadi kesalahan saat mengambil riwayat pemesanan kamu: {e}", "danger")
...
def upload_surat_izin(pesanan_id):
    try:
        ...
        if not pesanan:
            return jsonify({'success': False, 'message': 'Pesanan tidak ditemukan atau kamu tidak memiliki izin.'}), 404
...
def riwayat_pemesanan():
    ...
    try:
        ...
        app.logger.error(
            f"Terjadi kesalahan saat mengambil riwayat pemesanan kamu: {e}",
            exc_info=True,
        )
        flash(
            f"Terjadi kesalahan saat mengambil riwayat pemesanan kamu. Mohon coba lagi nanti.",
            "danger",
        )
        return render_template(
            "user/riwayat_pemesanan.html",
...
def ripe_diproses():
    ...
    try:
        ...
        diproses_statuses = ['Belum Pemotretan', 'Sudah Pemotretan']
        pesanan_cursor = db.pesanan.find({
            "user_id_pemesan": ObjectId(user_id),
        ...
        for pesanan in pesanan_cursor:
            ...
            # Menambahkan flag untuk melacak apakah jadwal sudah pernah diubah
            pesanan['has_rescheduled'] = pesanan.get('has_rescheduled', False)
...
def jadwal_ubah(pesanan_id):
    ...
    try:
        ...
        if not pesanan:
            flash("Pesanan tidak ditemukan atau kamu tidak memiliki izin untuk mengubahnya.", "danger")
            return redirect(url_for("ripe_diproses")) 

        allowed_statuses_for_edit = ['Belum Pemotretan', 'Sudah Pemotretan']
        if pesanan.get("status_pesanan") not in allowed_statuses_for_edit:
            flash(f"Pesanan dengan status '{pesanan.get('status_pesanan')}' tidak dapat diubah.", "warning")
            return redirect(url_for("ripe_diproses")) 
         ...
         # pengecekan apakah jadwal sudah pernah diubah
        if pesanan.get('has_rescheduled'): 
            flash("Jadwal pesanan ini hanya dapat diubah satu kali dan sudah dilakukan.", "warning") 
            return redirect(url_for("ripe_diproses"))
...
def admin_faq_tambah():
    ...
    # --- POST Request (Memproses data dari AJAX) ---
    if request.method == "POST":
        ...
        if faq_sudah_ada:
            return jsonify({
                'status': 'warning',
                'message': 'Pertanyaan ini sudah ada di dalam database.'
            }), 409 # Conflict
        ...
        try:
            ...
            return jsonify({
                'status': 'error',
                'message': 'Terjadi kesalahan internal saat menyimpan ke database.'
    ...
    if request.method == "GET":
        try:
            ...
            if not faq_untuk_diubah:
                # Jika tidak ditemukan, redirect tetap oke di sini
                return redirect(url_for("admin_faq", _sa_status='error', _sa_message="FAQ tidak ditemukan."))
            faq_untuk_diubah['_id'] = str(faq_untuk_diubah['_id'])
            return render_template("admin/faq_ubah.html", faq=faq_untuk_diubah)
        except Exception as e:
            return redirect(url_for("admin_faq", _sa_status='error', _sa_message=f"ID FAQ tidak valid: {e}"))

    # Bagian POST request diubah untuk mengembalikan JSON
    if request.method == "POST":
        ...
        if not pertanyaan_baru or not jawaban_baru:
            ...
            return jsonify({
                'status': 'error',
                'message': 'Pertanyaan dan jawaban tidak boleh kosong.'
            }), 400 # 400 adalah status HTTP untuk Bad Request

        # Cek duplikasi
        existing_faq_with_same_question = koleksi_faqs.find_one({
            "pertanyaan": {"$regex": f"^{pertanyaan_baru}$", "$options": "i"},
            "_id": {"$ne": ObjectId(id_faq)}
        })
        if existing_faq_with_same_question:
            # Kembalikan JSON warning
            return jsonify({
                'status': 'warning',
                'message': 'Pertanyaan ini sudah ada di FAQ lain.'
            }), 409 # 409 adalah status HTTP untuk Conflict
        ...
        try:
            ...
            return jsonify({
                'status': 'error',
                'message': 'Terjadi kesalahan internal saat mengubah FAQ.'
            }), 500 # 500 adalah status HTTP untuk Internal Server Error
...
def form_faq_user():
    """
    Menampilkan halaman form (GET) dan memproses data form via AJAX (POST).
    """
    if request.method == 'POST':
        ...
        if not nama_pengaju or not email_pengaju or not pertanyaan:
            # Mengembalikan pesan error sebagai JSON
            return jsonify({'status': 'error', 'message': 'Harap lengkapi semua kolom yang wajib diisi.'}), 400
        ...
        try:
            ...
            return jsonify({
                'status': 'success', 
                'message': 'Pertanyaan kamu berhasil dikirim!'
            })

        except Exception as e:
            print(f"Error saat menyimpan FAQ ke database: {e}")
            # Mengembalikan pesan error umum sebagai JSON
            return jsonify({'status': 'error', 'message': 'Terjadi kesalahan internal saat memproses permintaan Anda.'}), 500

    # Untuk request GET, tetap tampilkan halaman form seperti biasa
    data_input = {
...
def formulasan(pesanan_id):
    ...
    pesanan = db.pesanan.find_one({"_id": ObjectId(pesanan_id), "user_id_pemesan": ObjectId(user_id)})
    if not pesanan:
        flash("Pemesanan tidak ditemukan atau kamu tidak memiliki izin untuk mengulas pemesanan ini.", "danger")
        return redirect(url_for('ripe_selesai'))

    existing_review = reviews_collection.find_one({"pesanan_id": ObjectId(pesanan_id)})
    if existing_review:
        flash("Kamu sudah memberikan ulasan untuk pemesanan ini.", "info")
...
def api_lupa_kataSandi():
    ...
    try:
        msg = Message(
            "Ubah Kata Sandi Akun Oval Photo kamu",
...
def api_reset_password():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Kata sandi kamu berhasil diubah. Silakan masuk dengan kata sandi baru kamu."}), 200
...
def admin_logout():
    session.clear()
    flash("Kamu telah logout dari sisi admin.", "info")
    return redirect(url_for('admin_login'))
...
def pemilik_dashboard():
    """
    Menangani logika dan data untuk halaman dashboard pemilik.
    """
    try:
        # 1. LOGIKA FILTER TAHUN
        selected_year = int(request.args.get('year', datetime.now().year))
        ...
        # 2. LOGIKA DATA GRAFIK (Pendapatan per Bulan)
        start_of_year = datetime(selected_year, 1, 1)
        end_of_year = datetime(selected_year + 1, 1, 1)
        ...
        pendapatan_per_bulan_db = list(db.pesanan.aggregate(pipeline))
        
        data_map = {d['_id']['month']: d['total'] for d in pendapatan_per_bulan_db}

        chart_labels = ["Jan", "Feb", "Mar", "Apr", "Mei", "Jun", "Jul", "Ags", "Sep", "Okt", "Nov", "Des"]
        chart_data = [data_map.get(month, 0) for month in range(1, 13)]

        # 3. LOGIKA DATA KARTU STATISTIK
        today_start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
        jumlah_pesanan_hari_ini = db.pesanan.count_documents({"created_at": {"$gte": today_start}})
...
def api_pemilik_update_pengguna(user_id):
    ...
    try:
        ...
        # Perbarui password jika disediakan dan lebih dari 8 karakter
        if password:
            if len(password) < 8:
                return jsonify({"success": False, "message": "Kata sandi harus minimal 8 karakter."}), 400
            hashed_password = generate_password_hash(password, method='pbkdf2:sha256')
        ...
        if update_fields:
            ...
            return jsonify({"success": True, "message": "Pengguna berhasil diubah."}), 200
        else:
...
def create_midtrans_transaction():
    try:
        ...
        if str(pesanan.get('user_id_pemesan')) != session.get('user_id'):
            return jsonify({'success': False, 'message': 'Kamu tidak memiliki akses ke pesanan ini.'}), 403
        ...
        # mengambil data layanan berdasarkan layanan_id dari pesanan
        layanan_data = db.layanan.find_one({"_id": pesanan.get('layanan_id')})
        # mengambil data paket berdasarkan paket_id dari pesanan
        paket_data = db.paket.find_one({"_id": pesanan.get('paket_id')})

        # nama layanan, default 'N/A' jika tidak ditemukan
        nama_layanan = layanan_data.get('nama', 'N/A') if layanan_data else 'N/A'
        # nama paket, default 'N/A' jika tidak ditemukan
        nama_paket = paket_data.get('nama', 'N/A') if paket_data else 'N/A'
        ...
        if payment_type == 'deposit':
            ...
            item_details.append({
                ...
                "price": int(amount),
                "quantity": 1,
                "name": f"Pembayaran Deposit Layanan {nama_layanan} ({nama_paket})"
            })
            ...
            item_details.append({
                ...
                "price": int(amount),
                "quantity": 1,
                "name": f"Pembayaran Pelunasan Layanan {nama_layanan} ({nama_paket})"
            })
            ...
            item_details.append({
                ...
                "price": int(amount),
                "quantity": 1,
                "name": f"Pembayaran Sisa Layanan {nama_layanan} ({nama_paket})"
            })
        else:
...
def midtrans_payment_finish():
    ...
    if transaction_status == 'settlement' or transaction_status == 'capture':
        ...
        flash("Pembayaran kamu sedang menunggu konfirmasi. Silakan selesaikan pembayaran kamu.", "info")
    else:
